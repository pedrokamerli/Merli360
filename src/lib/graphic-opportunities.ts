...
export function validateOpportunityUpdate(input: { currentStatus: string; nextStatus?: string; lossReason?: string; nextAction?: string | null; nextFollowUp?: string | Date | null }) {
  const nextStatus = input.nextStatus || input.currentStatus;
  if (!isGraphicOpportunityStatus(nextStatus)) return "Status de oportunidade invalido.";
