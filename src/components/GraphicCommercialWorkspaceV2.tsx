...
export function GraphicCommercialWorkspaceV2({ scope = "all", onBackToLeads }: { scope?: "all" | "mine"; onBackToLeads?: () => void }) {
  ...
  const [pendingQuoteAction, setPendingQuoteAction] = useState<{ type: "send" | "approve" | "commercial" | "duplicate"; item: Row } | null>(null);
  ...
  async function approveCommercial(item: Row) {
    setSaving(true);
    const response = await fetch("/api/gestao-grafica/quotes", { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id: item.id, action: "approve-commercial", note: "Excecao comercial aprovada no CRM." }) });
    const body = await response.json();
    setSaving(false);
    if (!response.ok) return setMessage(body.error || "Nao foi possivel liberar a excecao comercial.");
    setPendingQuoteAction(null);
    setCreatedQuote((current) => current?.id === item.id ? { ...current, approvalRequired: false, approvalReason: null } : current);
    setMessage(`Orcamento #${item.number} liberado comercialmente. O link ja pode ser enviado ao cliente.`);
    await load();
  }
  ...
  async function confirmQuoteAction(event: FormEvent<HTMLFormElement>) {
    ...
    if (pendingQuoteAction.type === "commercial") await approveCommercial(pendingQuoteAction.item);
  ...
  return <div className="mx-auto max-w-screen-2xl space-y-5">
    ...
    {tab === "history" ? <section className="grid gap-4 xl:grid-cols-2"><section className="surface-panel p-4"><h2 className="mb-3 font-black text-slate-950">Orcamentos</h2><div className="space-y-2">{quotes.map((item: Row) => <article key={item.id} className="rounded-lg border border-slate-200 bg-white p-3"><div className="flex items-start justify-between gap-2"><div><p className="font-black text-slate-950">Orcamento #{item.number} <span className="text-slate-400">V{item.version || 1}</span></p><p className="text-xs font-semibold text-slate-500">{item.client?.name || "Cliente"} | {brl(item.totalPriceCents)}</p></div><span className="rounded-full bg-slate-100 px-2 py-1 text-xs font-black text-slate-600">{quoteStatus(item.status)}</span></div>{item.approvalRequired ? <div className="mt-3 rounded-lg border border-amber-200 bg-amber-50 p-3"><p className="text-xs font-black text-amber-900">Excecao comercial pendente</p><p className="mt-1 text-xs font-semibold text-amber-800">{item.approvalReason || "Revisao da margem necessaria antes do envio."}</p>{data?.canApproveQuotes ? <button className="secondary-action mt-2 inline-flex items-center gap-2 py-2 text-xs" type="button" disabled={saving} onClick={() => setPendingQuoteAction({ type: "commercial", item })}><CheckCircle2 size={14} />Aprovar excecao</button> : <p className="mt-2 text-xs font-black text-amber-800">Aguardando liberacao do Studium.</p>}</div> : null}<div className="mt-3 flex flex-wrap gap-2">{item.shareToken ? <button className="primary-action inline-flex items-center gap-2 py-2 text-xs" type="button" onClick={() => void copyQuoteLink(item)}>{copiedQuoteId === item.id ? <Check size={14} /> : <Copy size={14} />}Copiar link</button> : null}{item.shareToken ? <a className="secondary-action inline-flex items-center gap-2 py-2 text-xs" href={`/public/orcamento/${item.shareToken}`} target="_blank" rel="noreferrer"><ExternalLink size={14} />Abrir</a> : null}{item.shareToken ? <a className="secondary-action inline-flex items-center gap-2 py-2 text-xs" href={`/api/gestao-grafica/public-quotes/${item.shareToken}/pdf`} target="_blank" rel="noreferrer"><FileDown size={14} />PDF</a> : null}{item.shareToken && item.client?.phone && !item.approvalRequired ? <button className="secondary-action inline-flex items-center gap-2 py-2 text-xs" type="button" onClick={() => openQuoteWhatsApp(item)}><MessageCircle size={14} />WhatsApp</button> : null}{["DRAFT", "SENT", "REFUSED"].includes(item.status) ? <button className="secondary-action py-2 text-xs" type="button" disabled={saving} onClick={() => setPendingQuoteAction({ type: "duplicate", item })}>Nova versao</button> : null}{item.status === "DRAFT" ? <button className="secondary-action py-2 text-xs" type="button" disabled={saving || item.approvalRequired} onClick={() => { setSendFollowUp(todayPlus(1)); setPendingQuoteAction({ type: "send", item }); }}>Liberar e agendar retorno</button> : null}{["SENT", "VIEWED"].includes(item.status) && !item.approvalRequired ? <button className="primary-action inline-flex items-center gap-2 py-2 text-xs" type="button" disabled={saving} onClick={() => setPendingQuoteAction({ type: "approve", item })}><CheckCircle2 size={14} />Cliente aprovou</button> : null}</div></article>)}{!quotes.length ? <p className="rounded-lg bg-slate-50 p-4 text-sm font-bold text-slate-500">Nenhum orcamento criado.</p> : null}</div></section><section className="surface-panel p-4"><h2 className="mb-3 font-black text-slate-950">Pedidos aprovados</h2><div className="space-y-2">{orders.map((item: Row) => <article key={item.id} className="rounded-lg border border-slate-200 bg-white p-3"><p className="font-black text-slate-950">Pedido #{item.number}</p><p className="text-xs font-semibold text-slate-500">{item.clientName || "Cliente"} | {brl(item.soldValueCents)}</p><p className="mt-2 text-xs font-black text-emerald-700">Aprovado e integrado a producao</p></article>)}{!orders.length ? <p className="rounded-lg bg-slate-50 p-4 text-sm font-bold text-slate-500">Nenhum pedido aprovado.</p> : null}</div></section></section> : null}
    ...
    {pendingQuoteAction ? <Dialog title={pendingQuoteAction.type === "send" ? "Liberar orcamento" : pendingQuoteAction.type === "approve" ? "Confirmar aprovacao" : pendingQuoteAction.type === "commercial" ? "Aprovar excecao comercial" : "Criar nova versao"} onClose={() => setPendingQuoteAction(null)}><form className="grid gap-4" onSubmit={confirmQuoteAction}><div className="rounded-lg bg-slate-50 p-4"><p className="font-black text-slate-950">Orcamento #{pendingQuoteAction.item.number}</p><p className="mt-1 text-sm font-semibold text-slate-600">{pendingQuoteAction.item.client?.name || "Cliente"} | {brl(pendingQuoteAction.item.totalPriceCents)}</p></div>{pendingQuoteAction.type === "send" ? <label className="grid gap-1 text-xs font-black text-slate-600"><span>Data do proximo retorno</span><input className={inputClass} type="date" required value={sendFollowUp} onChange={(event) => setSendFollowUp(event.target.value)} /></label> : null}<p className="text-sm font-semibold text-slate-600">{pendingQuoteAction.type === "send" ? "O cliente podera abrir, baixar o PDF e aprovar pelo link." : pendingQuoteAction.type === "approve" ? "A aprovacao cria o pedido, os recebiveis e a ordem com as etapas de producao." : pendingQuoteAction.type === "commercial" ? `Esta decisao libera a excecao: ${pendingQuoteAction.item.approvalReason || "margem fora dos parametros"}.` : "Uma copia editavel sera criada como novo rascunho."}</p><div className="flex justify-end gap-2"><button className="secondary-action px-4 py-2" type="button" disabled={saving} onClick={() => setPendingQuoteAction(null)}>Cancelar</button><button className="primary-action px-4 py-2" disabled={saving}>{saving ? "Processando..." : pendingQuoteAction.type === "send" ? "Liberar para o cliente" : pendingQuoteAction.type === "approve" ? "Confirmar aprovacao" : pendingQuoteAction.type === "commercial" ? "Aprovar excecao" : "Criar versao"}</button></div></form></Dialog> : null}

    {createdQuote ? <QuoteShareDialog item={createdQuote} saving={saving} followUp={sendFollowUp} setFollowUp={setSendFollowUp} onSend={() => void sendQuote(createdQuote, sendFollowUp)} onCommercialReview={() => setPendingQuoteAction({ type: "commercial", item: createdQuote })} canApprove={Boolean(data?.canApproveQuotes)} onClose={() => setCreatedQuote(null)} onCopy={() => void copyQuoteLink(createdQuote)} copied={copiedQuoteId === createdQuote.id} onWhatsApp={() => openQuoteWhatsApp(createdQuote)} /> : null}
...
function QuoteShareDialog({ item, saving, followUp, setFollowUp, onSend, onCommercialReview, canApprove, onClose, onCopy, copied, onWhatsApp }: { item: Row; saving: boolean; followUp: string; setFollowUp: (value: string) => void; onSend: () => void; onCommercialReview: () => void; canApprove: boolean; onClose: () => void; onCopy: () => void; copied: boolean; onWhatsApp: () => void }) {
  const released = ["SENT", "VIEWED", "APPROVED"].includes(item.status) && !item.approvalRequired;
  return <Dialog title={`Orcamento #${item.number} pronto`} onClose={onClose}><div className="grid gap-4"><div className="rounded-lg border border-emerald-200 bg-emerald-50 p-4"><p className="text-xs font-black uppercase text-emerald-700">Total para o cliente</p><p className="mt-1 text-3xl font-black text-emerald-950">{brl(item.totalPriceCents)}</p><p className="mt-1 text-sm font-semibold text-emerald-800">{item.approvalRequired ? "Aguardando liberacao comercial antes do envio." : released ? "Link liberado para visualizar, baixar e aprovar." : "Defina o retorno e libere o orcamento para aprovacao."}</p></div>{item.approvalRequired ? <div className="rounded-lg border border-amber-200 bg-amber-50 p-4"><p className="font-black text-amber-950">Excecao comercial pendente</p><p className="mt-1 text-sm font-semibold text-amber-800">{item.approvalReason || "Margem fora dos parametros configurados."}</p>{canApprove ? <button className="secondary-action mt-3 inline-flex items-center gap-2 px-4 py-2" type="button" onClick={onCommercialReview}><CheckCircle2 size={16} />Aprovar excecao</button> : <p className="mt-2 text-xs font-black text-amber-800">Solicite a liberacao ao Studium.</p>}</div> : null}{!released && !item.approvalRequired ? <label className="grid gap-1 text-xs font-black text-slate-600"><span>Proximo retorno comercial</span><input className={inputClass} type="date" value={followUp} onChange={(event) => setFollowUp(event.target.value)} /></label> : null}<div className="grid gap-2 sm:grid-cols-2">{!released && !item.approvalRequired ? <button className="primary-action inline-flex items-center justify-center gap-2 px-4 py-3 sm:col-span-2" type="button" disabled={saving} onClick={onSend}>{saving ? <Loader2 className="animate-spin" size={17} /> : <Send size={17} />}Liberar link e agendar retorno</button> : null}<button className="secondary-action inline-flex items-center justify-center gap-2 px-4 py-3" type="button" onClick={onCopy}>{copied ? <Check size={17} /> : <Copy size={17} />}Copiar link</button><a className="secondary-action inline-flex items-center justify-center gap-2 px-4 py-3" href={`/api/gestao-grafica/public-quotes/${item.shareToken}/pdf`} target="_blank" rel="noreferrer"><FileDown size={17} />Abrir PDF</a><button className="primary-action inline-flex items-center justify-center gap-2 px-4 py-3 sm:col-span-2" type="button" disabled={!released} onClick={onWhatsApp}><MessageCircle size={17} />Enviar link pelo WhatsApp</button></div></div></Dialog>;
}
