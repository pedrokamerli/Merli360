...
import { Check, Copy, ExternalLink, Eye, EyeOff, ImageIcon, Loader2, Plus, RefreshCw, Save, Search, Upload } from "lucide-react";
...
export function GraphicCatalogWorkspace() {
  ...
  async function uploadImage() {
    if (!selected || !imageFile) return setNotice("Selecione uma foto JPG, PNG ou WebP.");
    setSaving(true);
    const form = new FormData();
    form.set("catalogItemId", selected.id);
    form.set("file", imageFile);
    const response = await fetch("/api/gestao-grafica/catalog/images", { method: "POST", body: form });
    const body = await response.json();
    setSaving(false);
    setNotice(response.ok ? "Foto atualizada no catalogo." : body.error || "Nao foi possivel enviar a foto.");
    if (response.ok) {
      setImageFile(null);
  ...
  return <div className="mx-auto max-w-screen-2xl space-y-5">
    ...
    {selected && data?.canManage ? <section className="surface-panel grid gap-4 p-5 lg:grid-cols-[280px_minmax(0,1fr)] lg:items-center"><div className="aspect-[16/10] overflow-hidden rounded-lg border border-slate-200 bg-slate-100" style={imageStyle(selected)} /><div><div className="flex items-center gap-2"><ImageIcon className="text-emerald-600" size={18} /><h2 className="font-black text-slate-950">Foto do produto</h2></div><p className="mt-1 text-sm font-semibold text-slate-500">Envie a foto correta. Ela substitui a imagem atual no catalogo e permanece salva no servidor.</p><label className="mt-3 block cursor-pointer rounded-lg border border-dashed border-slate-300 bg-slate-50 p-3 text-sm font-black text-slate-700"><input className="block w-full text-xs font-semibold file:mr-3 file:rounded-md file:border-0 file:bg-slate-950 file:px-3 file:py-2 file:font-black file:text-white" type="file" accept="image/jpeg,image/png,image/webp" onChange={(event) => setImageFile(event.target.files?.[0] || null)} /></label><div className="mt-3 flex flex-wrap items-center gap-3">{imageFile ? <button className="primary-action inline-flex items-center justify-center gap-2 px-4 py-2" type="button" disabled={saving} onClick={() => void uploadImage()}>{saving ? <Loader2 className="animate-spin" size={16} /> : <Upload size={16} />}Enviar e usar esta foto</button> : null}<span className="text-xs font-semibold text-slate-500">{selected.imageOriginalName ? `Atual: ${selected.imageOriginalName}` : "JPG, PNG ou WebP, ate 10MB."}</span></div></div></section> : null}
