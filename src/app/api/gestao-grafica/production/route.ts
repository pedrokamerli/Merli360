...
export async function PUT(request: NextRequest) {
  try {
    ...
    return NextResponse.json({ error: "Nao foi possivel atualizar a producao.", detail: process.env.NODE_ENV === "production" ? undefined : String(error?.message || error) }, { status: error?.message === "UNAUTHORIZED" ? 401 : 500 });
  }
}
