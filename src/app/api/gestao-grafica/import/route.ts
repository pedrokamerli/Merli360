    ...
    const result = await db.$transaction(async (tx: any) => {
      ...
      for (const item of preview.items) {
        if (item.type === "product") {
          ...
          if (existing) {
            await tx.graphicProduct.update({ where: { id: existing.id }, data: { category: item.category, unit: item.unit, description: item.description || existing.description, validationStatus: "PENDING_VALIDATION", updatedById: user.id } });
            counters.productsUpdated++;
          } else {
            await tx.graphicProduct.create({ data: { tenantId: user.tenantId, name: item.name, category: item.category || "Grafica", unit: item.unit || "unidade", description: item.description || null, status: "ACTIVE", validationStatus: "PENDING_VALIDATION", createdById: user.id, updatedById: user.id } });
        ...
        if (item.type === "material") {
          ...
          if (existing) {
            await tx.graphicMaterial.update({ where: { id: existing.id }, data: { unit: item.unit || existing.unit, currentCostCents: item.costCents || 0, wastePercent: item.wastePercent || 0, validationStatus: "PENDING_VALIDATION", updatedById: user.id } });
            ...
            const created = await tx.graphicMaterial.create({ data: { tenantId: user.tenantId, name: item.name, unit: item.unit || "unidade", currentCostCents: item.costCents || 0, wastePercent: item.wastePercent || 0, status: "ACTIVE", validationStatus: "PENDING_VALIDATION", createdById: user.id, updatedById: user.id } });
            await tx.graphicMaterialCostHistory.create({ data: { tenantId: user.tenantId, materialId: created.id, costCents: item.costCents || 0, source: `IMPORT:${file.name}:${item.sheet}:${item.rowNumber}`, status: "PENDING_VALIDATION", createdById: user.id, updatedById: user.id } });
        ...
        if (item.type === "process") {
          ...
          if (existing) {
            await tx.graphicProcess.update({ where: { id: existing.id }, data: { type: item.processType || existing.type, unit: item.unit || existing.unit, costCents: item.costCents || 0, validationStatus: "PENDING_VALIDATION", updatedById: user.id } });
            counters.processesUpdated++;
          } else {
            await tx.graphicProcess.create({ data: { tenantId: user.tenantId, name: item.name, type: item.processType || "INTERNAL", unit: item.unit || "hora", costCents: item.costCents || 0, status: "ACTIVE", validationStatus: "PENDING_VALIDATION", createdById: user.id, updatedById: user.id } });
