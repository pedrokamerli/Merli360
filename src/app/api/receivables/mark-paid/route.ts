...
import { syncReceivablePayment } from "@/lib/transaction-sync";
...
export async function POST(request: NextRequest) {
  ...
  const receivable = await prisma.accountReceivable.findFirst({ where: { id: body.id, tenantId: user.tenantId } });
  if (!receivable) return NextResponse.json({ error: "Conta a receber nao encontrada" }, { status: 404 });

  const item = await syncReceivablePayment(receivable, {
