...
export default async function PublicQuotePage({ params }: { params: Promise<{ token: string }> }) {
  ...
  const quote = await (prisma as any).graphicQuote.findFirst({
    where: { shareToken: token, status: { in: ["PENDING_REVIEW", "DRAFT", "SENT", "VIEWED", "APPROVED"] } },
    include: { tenant: true, items: true, orders: { include: { productionOrders: { include: { steps: { orderBy: { position: "asc" } } } }, deliveries: true } } }
  });
  if (!quote) notFound();
